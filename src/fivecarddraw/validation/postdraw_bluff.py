...
def run_analysis(
    *,
    ...
    node_weighted = [d for d in weighted if on_raise_node_pre_c(d)]
    if progress:
    ...
    node_extra = [d for d in extra if on_raise_node_pre_c(d)]
    ...
    return {
        "meta": {
            ...
            "node": "pre-Stage-C: BN two_pair+ (bets) ∩ caller straight+ (raises)",
            "ring": 1,
            "value_3bet": "flush+",
